...
export default function ChatPage() {
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    const isFirstMessage = !chat?.interactions || chat.interactions.length === 0
    ...
    <div className="flex flex-col h-screen max-h-screen p-2 box-border">
      ...
      <Card className="flex flex-col h-full bg-[#282828] border border-[#282828] shadow-none max-w-6xl mx-auto w-full">
        ...
        {hasMessages && (
          <CardFooter
            ...
            style={{
              animation: showMessages ? "slideInBottom 0.8s cubic-bezier(0.4, 0, 0.2, 1) 0.6s both" : "none",
            }}
          >
            <form onSubmit={handleSendMessage} className="w-full max-w-[700px] mx-auto">
              <div className="relative w-full">
                <Input
                  ref={inputRef}
                  value={input}
      ...
      <style jsx>{`
        ...
        @keyframes greetingExit {
          ...
          50% { opacity: 0.7; transform: translateY(-10px) scale(1.02); }
        ...
        @keyframes inputExit {
          0% { opacity: 1; transform: translateY(0) scale(1); }
          30% { opacity: 0.8; transform: translateY(10px) scale(0.98); }
